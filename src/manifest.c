 ...
 *      gcc -std=c17 -Wall -Wextra -pedantic -o showcase c_features_showcase.c -lm -lpthread
 *
 *  Table of Contents:
...
#define SWAP(a, b, T)   do { T tmp_ = (a); (a) = (b); (b) = tmp_; } while (0)
#define PRINT_VAR(var)  printf("  %-20s = %d\n", #var, (var))  /* Stringification */
...
#define SHOW_PREDEFINED_MACROS() do {                                       \
    printf("  __FILE__         = %s\n", __FILE__);                          \
    printf("  __LINE__         = %d\n", __LINE__);                          \
    printf("  __DATE__         = %s\n", __DATE__);                          \
    printf("  __TIME__         = %s\n", __TIME__);                          \
...
static void demo_enums(void) {
    ...
    /* Enum used in switch */
    switch (favorite) {
        case RED:   printf("  Favorite: Red\n");   break;
        case GREEN: printf("  Favorite: Green\n"); break;
        case BLUE:  printf("  Favorite: Blue\n");  break;
    }
}
...
static void demo_structs_unions(void) {
    ...
    /* Bit-fields */
    struct PackedFlags flags = { .is_visible = 1, .is_enabled = 0, .priority = 7, .mode = 2 };
    ...
    struct {
        union {
            struct { int x; int y; };  /* anonymous struct inside anonymous union */
            int coords[2];
        };
        int z;
    } anon = { .x = 10, .y = 20, .z = 30 };
    printf("  Anonymous struct/union: x=%d y=%d coords={%d,%d} z=%d\n",
           anon.x, anon.y, anon.coords[0], anon.coords[1], anon.z);
#else
...
static void demo_pointers(void) {
    ...
    printf("  NULL pointer: %p, is null: %s\n",
           (void *)null_ptr, null_ptr == NULL ? "yes" : "no");
...
static void demo_memory(void) {
    ...
    if (!zeroed) { perror("calloc"); free(arr); return; }
...
static void demo_control_flow(void) {
    ...
    switch (level) {
        case 3: printf("admin ");   /* falls through */
        case 2: printf("write ");   /* falls through */
        case 1: printf("read ");    /* falls through */
        case 0: printf("guest");
                break;
...
static void print_doubled(int x) {
    printf("%d ", x * 2);
}
...
static void demo_storage_classes(void) {
    ...
    tls_var = 77;
    printf("  _Thread_local: %d (thread-local storage, each thread has own copy)\n", tls_var);
#else
...
static void demo_bitwise(void) {
    ...
    printf("  a & b    = "); print_binary(a & b, 8); printf("  (AND)\n");
    printf("  a | b    = "); print_binary(a | b, 8); printf("  (OR)\n");
    printf("  a ^ b    = "); print_binary(a ^ b, 8); printf("  (XOR)\n");
    printf("  ~a       = "); print_binary((unsigned char)~a, 8); printf("  (NOT)\n");
    printf("  a << 2   = "); print_binary(a << 2, 10); printf("  (left shift)\n");
    printf("  a >> 2   = "); print_binary(a >> 2, 8); printf("  (right shift)\n");

    /* Common bit manipulation idioms */
    unsigned int flags = 0;
    flags |= (1 << 3);   /* Set bit 3 */
    flags |= (1 << 5);   /* Set bit 5 */
    printf("\n  Set bits 3,5 : "); print_binary(flags, 8); printf("\n");

    flags &= ~(1 << 3);  /* Clear bit 3 */
    printf("  Clear bit 3  : "); print_binary(flags, 8); printf("\n");

    flags ^= (1 << 5);   /* Toggle bit 5 */
    printf("  Toggle bit 5 : "); print_binary(flags, 8); printf("\n");
    ...
    int count = 0;
    for (unsigned int tmp = v; tmp; tmp >>= 1) count += tmp & 1;
...
static void demo_file_io(void) {
    ...
    char name[32]; int age; float height;
...
static void demo_error_handling(void) {
    ...
    printf("  atexit: register cleanup functions (not demo'd to avoid side effects)\n");
}
...
static void demo_alignment(void) {
    ...
    printf("  alignas(16) int at %p (mod 16 = %zu)\n",
           (void *)&aligned_var, (uintptr_t)&aligned_var % 16);
    printf("  alignas(32) buf at %p (mod 32 = %zu)\n",
           (void *)aligned_buf, (uintptr_t)aligned_buf % 32);
#else
...
static void demo_atomics(void) {
    ...
    /* val was 15, counter is 15, so CAS should fail (val was updated by fetch_add) */
    /* Let's try with correct expected value */
    val = 15;
    ok = atomic_compare_exchange_strong(&atomic_counter, &val, 99);
    printf("  CAS(15→99): %s, counter=%d\n",
           ok ? "success" : "failed", atomic_load(&atomic_counter));

    printf("  Memory orders: relaxed, consume, acquire, release, acq_rel, seq_cst\n");
    ...
    printf("  Note: <threads.h> support varies; POSIX pthreads often used instead\n");
#else
...
static ListNode *list_push(ListNode *head, int data) {
    ...
    if (node) { node->data = data; node->next = head; }
    return node;
}
...
static void demo_datetime(void) {
    ...
    for (int i = 0; i < 1000000; i++) dummy += 0.001;
    clock_t end = clock();
...
static void demo_preprocessor_bonus(void) {
    ...
    _Pragma("message(\"This is a _Pragma() operator demo (seen during compilation)\")")
...
static void demo_noreturn(void) {
    ...
    printf("  Example: _Noreturn void fatal_error(const char *msg) { abort(); }\n");
    printf("  Standard _Noreturn functions: abort(), exit(), _Exit(), quick_exit()\n");
...
static void demo_wide_chars(void) {
    ...
    for (int i = 0; test[i]; i++) {
        wchar_t ch = test[i];
        printf("L'%lc'(", ch);
        if (iswalpha(ch)) printf("alpha");
        else if (iswdigit(ch)) printf("digit");
        else if (iswspace(ch)) printf("space");
        else if (iswpunct(ch)) printf("punct");
        printf(") ");
    }
    printf("\n");
...
static void demo_tgmath(void) {
    ...
    printf("  <tgmath.h> provides type-generic macros for <math.h> + <complex.h>\n");
    printf("  When included, functions like sqrt(), fabs(), pow() become\n");
    printf("  type-generic: they dispatch to the correct variant based on type:\n");
    printf("\n");
...
static void demo_locale(void) {
    ...
    char saved[128];
    if (original) strncpy(saved, original, sizeof(saved) - 1);
...
static void demo_iso646(void) {
    ...
    if (a and b) printf("  a and b       : true (&&)\n");
    if (a or b)  printf("  a or b        : true (||)\n");
    if (not 0)   printf("  not 0         : true (!)\n");
    ...
    /* not_eq (!=) */
    if (a not_eq b) printf("  a not_eq b    : true (!=)\n");
}
...
/* 'static' in array parameter tells the compiler the array has at least N elements.
 * This enables optimizations and can produce warnings if violated. */
static int sum_at_least_4(int arr[static 4]) {
    return arr[0] + arr[1] + arr[2] + arr[3];
}

/* Also works with multidimensional arrays and qualifiers */
static void fill_matrix(int rows, int cols, int mat[static rows][cols], int val) {
    for (int i = 0; i < rows; i++)
        for (int j = 0; j < cols; j++)
            mat[i][j] = val;
}
